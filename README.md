...
**🖨️ Forms & Output (1)** — `xdp-form-layout` (project-wide Adobe Forms/XDP layout standard: A4 portrait/landscape frame, margins, 5-area model top/header/body/signature/footer, Roboto typography, binding & FormCalc conventions + text XDP skeleton in `references/` — every form same frame, only body data differs).
