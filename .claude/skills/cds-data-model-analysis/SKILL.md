...
description: Analyze and design the data model BEHIND a complex report or CDS view — identify keys, foreign-key relationships, correct join/association conditions, cardinality, and fan-out/duplication risks across SAP released I_* CDS views and custom Z-tables, then design the join tree before any CDS syntax is written. Use this skill whenever a report needs data from 2+ tables/views and the relationships between them are not yet pinned down, when the user asks how tables/views relate, which fields to join on, why a report shows duplicate rows or inflated totals, what the cardinality between two entities is, or how to determine a CDS view's keys and associations. Triggers include "join các bảng", "xác định key/foreign key", "quan hệ giữa các bảng", "design join for report", "duplicate rows in report", "cardinality", "phân tích cấu trúc CDS view", "build join tree". For writing the actual CDS join/aggregation syntax use cds-analytical-views; for RAP composition trees use cds-view-entities; for finding which released I_* view to use in the first place use find-released-cds-view.
---
