---
name: rap
description: Help with RAP (RESTful ABAP Programming Model) development including behavior definitions, EML statements, managed and unmanaged BOs, draft handling, actions, validations, determinations, and side effects. Use when users ask about RAP, BDEF, BDL, EML, behavior definitions, behavior pools, managed BO, unmanaged BO, draft-enabled BO, RAP actions, RAP validations, RAP determinations, RAP side effects, create/read/update/delete in RAP, or building transactional Fiori apps with ABAP Cloud. Triggers include "create a RAP BO", "write a behavior definition", "EML syntax", "managed vs unmanaged", "enable draft", "add an action", "add a validation", "RAP handler method", or "RAP saver class". For event bindings, Event Mesh integration, and event consumption use rap-business-events; for custom entity query providers (IF_RAP_QUERY_PROVIDER) use rap-query-provider; for OData protocol/publishing specifics use odata. Use even for small single-prompt asks/fixes, not only full builds — Vietnamese triggers: "thêm action", "sửa behavior definition", "viết validation/determination", "xử lý draft", "sửa handler".
---
