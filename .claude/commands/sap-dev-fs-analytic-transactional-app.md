...
1.0 NEW TABLE DESIGN + RELEASED CDS VALIDATION: Dispatch `[Agent: consultant-lead]` ONCE, naming all three sub-tasks below in the same dispatch prompt to run in sequence inside that one agent — do not split into separate dispatches: (b) strictly needs (a)'s composition tree, and (c) feeds the same combined Verified Data Model output, so nothing here parallelizes and splitting them only adds round-trip overhead.
